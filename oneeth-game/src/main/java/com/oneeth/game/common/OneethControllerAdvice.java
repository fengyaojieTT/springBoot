 ...
 */
@ControllerAdvice
public class OneethControllerAdvice {
    ...
    public void addAttributes(Model model) {
        model.addAttribute("author", "Mr.Feng");
    }
    ...
    public  OneethResponse<?> errorHandler(Exception ex) {
        ex.printStackTrace();
        logger.error(ex.getMessage(),ex);
        return ResponseFormat.retParam(00000,"");
    }

    /**
     * 拦截捕捉自定义异常 OneethException.class
     * @param ex
     * @return
     */
    @ResponseBody
    @ExceptionHandler(value = OneethException.class)
    public OneethResponse<?> myErrorHandler(OneethException ex) {
        ex.getException().printStackTrace();
        logger.error(ex.getException().toString());
        return  ResponseFormat.retParam(ex.getCode(),"");
    }
}